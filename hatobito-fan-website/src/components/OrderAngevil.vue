<template>
  <div class="koiiro-order-form">
    <h2 class="text-xl font-bold mb-4">ANGeVIL Civil War Goods</h2>
    ...
    <div class="overflow-x-auto">
      <table class="w-full min-w-max text-sm">
        ...
        <tbody>
          <tr v-for="(item, index) in ordersAngevil" :key="index" class="md:table-row">
            <!-- Goods -->
...
const grandTotal = computed(() => {
  return ordersAngevil.value.reduce((sum, item) => sum + getTotalPrice(item), 0);
});
...
const ordersAngevil = ref([{ product: "", member1: "", quantity: 1 }]);

onMounted(() => {
  const savedOrders = localStorage.getItem("ordersAngevil");
  if (savedOrders) {
    ordersAngevil.value = JSON.parse(savedOrders);
  }
});
...
watch(
  ordersAngevil,
  (newOrdersAngevil) => {
    localStorage.setItem("ordersAngevil", JSON.stringify(newOrdersAngevil));
  },
  { deep: true }
);

const products = [
  { name: "HeARTBReAK POSTER", price: 500, memberCount: 0, memberType: "members" },
  { name: "Race Me POSTER", price: 500, memberCount: 0, memberType: "members" },
  { name: "ANGeVIL Member POSTER", price: 350, memberCount: 1, memberType: "members" },
  { name: "ANGeVIL Cheki", price: 300, memberCount: 1, memberType: "members" },
];
...
function addOrder() {
  ordersAngevil.value.push({ product: "", member1: "", quantity: 1 });
}

function removeOrder(index) {
  ordersAngevil.value.splice(index, 1);
}
